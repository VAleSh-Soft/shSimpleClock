...
В часах используются три [кнопки](docs/buttons.md) для [настройки](docs/setting.md) поддерживаемых опций. Так же есть возможность использования до двух дополнительных кнопок.
...
Часы имеют собственный [диспетчер задач](docs/api.md#диспетчер-задач), который позволяет также добавлять пользовательские задачи, которые будут выполняться наряду со штатными.
...
***Важно!!!** - объявление файла **clockSetting.h** в скетче должно быть **ДО** объявления библиотеки **shSimpleClock.h** (смотри пример ниже)*

Простейший скетч часов выглядит так:
```
 ...
 * исправлено поведение светодиодного индикатора будильника - не мигал при срабатывании будильника;
 + добавлен метод `void resetButtonState(clkButtonType _btn)`, позволяющий обнулить внутренние счетчики состояния кнопки;
 + добавлен метод `bool isButtonClosed(clkButtonType _btn)`, который возвращает true, если по результатам последнего опроса кнопка `_btn` нажата;
 + добавлен метод `bool getBlink()`, позволяющий использовать блинк часов, например, на пользовательских экранах;
 + добавлены методы, позволяющие добавлять пользовательские задачи в список диспетчера задач библиотеки;
 + добавлен пример работы с пользовательскими задачами ([additional_task.ino](examples/other_examples/additional_task/additional_task.ino));
 * доработан и исправлен код встроенного диспетчера задач;
 * исправлена ошибка, приводившая к ряду ошибок в работе встроенных задач библиотеки (автовозвращение в режим по умолчанию, сигнал будильника и т.д.);
 + добавлена лицензия;
 + добавлен метода `void setTaskState(clkHandle _handle, bool _state);` для управления пользовательскими задачами;
 * мелкие и не очень мелкие исправления и доработки;

Версия 1.8 - 05.01.2026
 ...
 * исправлена проблема с неустойчивой работой экрана **TM1637** на **BluePill** при использовании аддона **Arduino_STM32 by Roger Clark**, если часы используют аналоговые датчики - датчик освещенности и/или NTC-термистор как датчик температуры;
 + добавлен пример использования библиотеки на **STM32** ([stm32.ino](examples/various_mcu/stm32/stm32.ino));
 ...
 * решена проблема неустойчивой работы текстовых экранов **LCD1602/2004** на платах **BluePill**;
 + для контроллеров **ESP8266**, **ESP32**, **RP2040** добавлена возможность устанавливать размер используемой `EEPROM` (в байтах);
 * исправлена ошибка, приводящая к миганию экрана в момент смены яркости;
 * мелкие исправления и доработки;

Версия 1.7 - 23.05.2024
 + добавлена поддержка **RP2040**;
 + добавлен пример использования библиотеки на **RP2040** ([rp2040.ino](examples/various_mcu/rp2040/rp2040.ino));
...
Версия 1.6 - 19.05.2024
 ...
 + добавлен пример использования библиотеки с экраном **LCD 1602** ([lcd1602_i2c.ino](examples/various_displays/lcd1602_i2c/lcd1602_i2c.ino));
 ...
 + добавлен пример использования библиотеки с экраном **LCD 2004** ([lcd2004_i2c.ino](examples/various_displays/lcd2004_i2c/lcd2004_i2c.ino));
 * мелкие исправления и доработки;
...
Версия 1.3 - 26.03.2024
 ...
 + добавлен пример работы с пользовательским режимом экрана ([custom_screen.ino](examples/other_examples/custom_screen/custom_screen.ino));
...
***Важно!!!** - при обновлении библиотеки не забывайте проверять файлы **clockSetting.h** в ваших проектах на предмет появления новых опций и изменения типов старых; если после обновления библиотеки у вас что-то работает неправильно, спомните этот совет.*

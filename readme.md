...
2. Текстовые экраны **LCD 1602/2004** могут работать неустойчиво от экземпляра к экземпляру, при чем, один и тот же экран может глючить с одной платой и нормально работать с другой и наоборот; проблема была выявлена при использовании **BluePill** с аддоном **stm32duino**; с аддоном **Arduino_STM32** экраны отказываются работать напрочь; с этим разбираться пока тоже некогда :

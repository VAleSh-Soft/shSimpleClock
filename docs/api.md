...
```
позволяет установить другой режим. Например, код
```
clock.setDisplayMode(DISPLAY_MODE_SHOW_TEMP);
```
выведет на экран часов текущую температуру (если эта опция используется).
...
```
зеркальное отражение по горизонтали (построчно). Дело в том, что, если вы используете готовые китайские матрицы на четыре модуля 8х8, например:
...
```
позволяет получить текущие дату и время. Методы
```
...
```
позволяют узнать, включен или нет будильник, или включить/отключить его.

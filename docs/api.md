...
Диспетчер задач библиотеки позволяет добавлять в свой список пользовательские задачи, и они будут обрабатываться наравне с системными задачами часов. 

Для использования этой возможности нужно указать количество задач, которые предполагается использовать, в блоке [Прочие настройки](clock_setting.md#блок-прочие-настройки) в файле **clockSetting.h** в строке `#define ADDITIONAL_TASKS_COUNT 0`. 

У пользователя есть возможность управления своими задачами - запуск, остановка, получение и изменение статуса задачи, а также изменение интервала ее выполнения. Для этого используются следующие методы:
```
...
```
запуск задачи с идентификатором `_handle`; если применить метод к уже исполняющейся задаче, то она будет перезапущена, т.е. таймер ее исполнения будет обновлен текущим моментом времени;

```
void stopTask(clkHandle _handle);
```
остановка задачи с идентификатором `_handle`;

```
bool getTaskState(clkHandle _handle);
```
получение статуса задачи с идентификатором `_handle`; возвращает **true**, если задача активна (выполняется) или **false**, если задача неактивна (остановлена) или не существует;

```
void setTaskState(clkHandle _handle, bool _state);
```
установка статуса задачи; здесь:
- `_handle` - идентификатор задачи;
- `_state` - новый статус задачи; при **true** (по умолчанию) задача начнет выполняться (или будет перезапущена, если уже была активна) с этого момента, при **false** - задача будет остановлена;
...
void setTaskInterval(clkHandle _handle,
                                 ...
                                 bool _restart = true);
```
установка нового интервала срабатывания задачи; здесь:
- `_handle` - идентификатор задачи;
...
```
немедленное выполнение функции, привязанной к задаче; здесь:
- `_handle` идентификатор задачи;

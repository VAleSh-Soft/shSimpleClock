...
```
позволяет включать и отключать анимацию вывода информации на экран. Метод доступен для матричных экранов при использовании опции `USE_TICKER_FOR_DATA`
...
Метод доступен для матричных экранов при использовании опций `USE_CALENDAR` или `USE_TEMP_DATA`.
...
Библиотека позволяет задавать callback-функции на свои события. Всего есть два события: ежесекундное событие и событие будильника. Ежесекундное событие, как следует из его названия, происходит каждый раз при смене секунды. К нему относятся методы:
```

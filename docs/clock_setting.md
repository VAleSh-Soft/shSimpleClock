...
Для экранов на базе **TM1637** и **MAX7219**/**MAX7221** в этом блоке нужно указать только номера пинов, к которым будут подключаться экраны. При этом имейте в виду, что драйвера **MAX7219**/**MAX7221** работают через аппаратный **SPI** вашего микроконтроллера, и не каждый МК позволяет их переназначать. 
...
Номера пинов соответствуют номерам пинов аппаратного **I2C** выбранного МК, к которым подключен модуль **RTC**, приведены только для информации и должны совпадать с [таковыми](#блок-модуль-rtc), заданными для модуля **RTC**;

#### Матрица из адресных светодиодов

Предполагается, что в качестве экрана используется матрица **8х32** (8 строк, 32 столбца).
...
```
указывает, что для вывода дней недели и прочей информации будут использоваться символы кириллицы. Если вам это не нужно, закомментируйте эту строку, информация будет выводиться латиницей.
...
задают настройки кнопок:
 ...
 - интервал выдачи событий **BTN_LONGCLICK** при удержании кнопки нажатой (используется для кнопок **Up** и **Down**, определяет скорость смены значений при удержании кнопок), 
 - интервал антидребезга, 
 - интервал двойного клика. 

Все настройки задаются в милисекундах.

***Важно!!!** - интервал обновления интерфейсов настроек составляет 50 милисекунд, соответственно, минимальный интервал выдачи событий **BTN_LONGCLICK** может составлять именно 50 милисекунд; поэтому значение `INTERVAL_OF_SERIAL` нужно задавать кратным 50, т.е. 50, 100, 150; если же задавать какие-то промежуточные значения, то возможна визуальная неравномерность изменения значений.*

Строка 
```
#define USE_BUZZER_FOR_BUTTON
```
определяет, будет ли озвучиваться каждый клик кнопки. Это может быть полезно при использовании сенсорных кнопок. Конечно, для этого в системе должна присутствовать пищалка (или другой источник звука), и должен быть определен пин, к которому она подключена.
...
```
если какая-то из кнопок не будет использоваться, вместо номера нужно задать -1.
...
```
позволяет входить в настройки будильника по однократному клику кнопкой **Set** вместо двойного клика, заданного по умолчанию.
...
Строка
```
#define ADDITIONAL_TASKS_COUNT 0
```
задает задает количество дополнительных (пользовательских) задач, которые будут использоваться вместе с основными; о работе с диспетчером задач можно посмотреть [здесь](api.md#диспетчер-задач)

### Блок "настройки EEPROM"

Здесь задаются адреса ячеек в EEPROM для сохранения настроек часов.
...
Модули работают через аппаратный **I2C** микроконтроллера, соответственно, пины аппаратного **I2C** выбранного МК и нужно указывать.
